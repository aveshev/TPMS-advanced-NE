...
# Horizontal squeeze (3rd arg): the source is wider than a real MP3/Tricity
sx=s*(float(sys.argv[3]) if len(sys.argv)>3 else 1)
# output -> source affine at 4x
big=Image.fromarray(src.astype(np.uint8)).transform((W*K,H*K),Image.AFFINE,(1/(sx*K),0,cx-312.5/sx,0,1/(s*K),top-8/s),resample=Image.BICUBIC)
b=np.array(big)/255
...
hx,hy=T(404.3,826)
hole=f'<ellipse cx="{hx:.1f}" cy="{hy:.1f}" rx="{1.9*sx*K:.1f}" ry="{4.6*s*K:.1f}" transform="rotate(-8 {hx:.1f} {hy:.1f})" stroke-width="{1.3*K}"/>'
